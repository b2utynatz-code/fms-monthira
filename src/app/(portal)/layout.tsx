    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <header className="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <Link href="/portal" className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-primary flex items-center justify-center text-primary-foreground font-bold text-lg shadow-sm">
              F
            </div>
            <div>
              <div className="font-bold text-base leading-none tracking-tight">
                {locale === "en" ? "Faculty of Management Sciences" : "คณะวิทยาการจัดการ"}
              </div>
      ...
      <footer className="border-t bg-muted/40 py-12 text-sm">
        <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs text-muted-foreground">
              <li><Link href="/portal/curriculum" className="hover:text-primary">{locale === "en" ? "Curriculum" : "หลักสูตรทั้งหมด"}</Link></li>
              <li><Link href="/portal/staff" className="hover:text-primary">{locale === "en" ? "Faculty Members" : "ทำเนียบอาจารย์"}</Link></li>
              <li><Link href="/portal/news" className="hover:text-primary">{locale === "en" ? "Announcements" : "ประกาศและข่าวสาร"}</Link></li>
              <li><Link href="/portal/bookings" className="hover:text-primary">{locale === "en" ? "Room Schedule" : "ปฏิทินห้องประชุม"}</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs text-muted-foreground">
              <li><Link href="/login" className="hover:text-primary">{locale === "en" ? "Admin Console" : "ระบบหลังบ้านคณะ (Admin)"}</Link></li>
              <li><Link href="/documents" className="hover:text-primary">{locale === "en" ? "Document Workflow" : "ระบบสารบรรณและคำขอ"}</Link></li>
              <li><Link href="/bookings" className="hover:text-primary">{locale === "en" ? "Resource Reservations" : "ระบบจองห้องและยานพาหนะ"}</Link></li>
            </ul>
          </div>
        ...
        <div className="container mx-auto px-4 mt-8 pt-6 border-t text-center text-xs text-muted-foreground">
          © {new Date().getFullYear()} Faculty of Management Sciences. Powered by VibeCore Framework.
        </div>
      </footer>
    </div>
  );
